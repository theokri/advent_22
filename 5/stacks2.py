...
def perform_move(stacks: list[list[str]], move: tuple[int, int, int]) -> list[list[str]]:
    ...
    if n == 1:
        ...
        starting_crate = len(stacks[from_stack]) - n
        cargo_to_move = stacks[from_stack][starting_crate:]
        stacks[from_stack] = stacks[from_stack][:starting_crate]
        stacks[to_stack].extend(cargo_to_move) # extend maintains order of cargo to move
    return stacks
